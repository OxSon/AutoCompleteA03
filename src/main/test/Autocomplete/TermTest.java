...
class TermTest {
    ...
    void byReverseWeightOrder() {
        ...
        assertNotEquals(wiki, wikiRandomized);
        ...
        for (int i = 0; i < wiki.length; i++) {
            assertEquals(wiki[i].weight(), wikiRandomized[i].weight());
        }
    }

    @Test
    void byPrefixOrder() {
        assertThrows(IllegalArgumentException.class, () -> Term.byPrefixOrder(-3));
        Object[][] testData = {
    ...
    void compareTo() {
        ...
        assertEquals(term2.compareTo(term1), 0);
        //   -A shorter than B
